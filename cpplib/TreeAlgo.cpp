    ...
    namespace tree {
        ...
        public:
            LCA(vector<vector<int>>& g, int root = 0){
                dfs = DFS(g, 0);
                vector<Node> v(g.size());
                for(int i = 0; i < g.size(); i++){
                    v[i].value = dfs.eulerTour()[i];
                    v[i].depth = dfs.depth[v[i].value];
                }
                st = shiroha::datastructure::AnySparseTable<Node, nodemin>(v);
            }
        ...
        private:
            struct Node{
                ...
                int depth;
                DFS dfs;
                shiroha::datastructure::AnySparseTable<Node, nodemin> st;
            };

            Node nodemin(Node a, Node b){
                if(a.depth < b.depth){
                    return a;
                }else{
                    return b;
