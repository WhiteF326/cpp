...
int main(){
  string s, t; cin >> s >> t;
  map<string, int> s_app, t_app;
  vector<int> s_al(s.length()), t_al(s.length());

  for(int i = 0; i < s.length(); i++){
    if(s_app[s.substr(i, 1)]) s_al[i] = s_app[s.substr(i, 1)];
    else{
      s_al[i] = s_app.size();
      s_app[s.substr(i, 1)] = s_app.size();
    }
  }

  for(int i = 0; i < t.length(); i++){
    if(t_app[t.substr(i, 1)]) t_al[i] = t_app[t.substr(i, 1)];
    else{
      t_al[i] = t_app.size();
      t_app[t.substr(i, 1)] = t_app.size();
    }
  }
