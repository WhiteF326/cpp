...
int main() {
    ...
    // 次数 2, 2 のペア = 2
    // 次数 2, 3 のペア = 4
    // 次数 3, 3 のペア = 残り全部(3n - 8)

    arbitrary_mod_binomial comb(p);
    // i 列目まで見ていて、取り除き方の bit が j であり、今までに取り除いた辺は k
    ll dp[n][8][n + 1] = {0};
    dp[0][0][0] = 1;
    for(int i = 0; i < n; i++){
        for(int k = 0; k <= n; k++){
            for(int j = 0; j < 9; j++){
                if((j & 1) || (j & 2)){
                    dp[i + 1][6][k + 1] += dp[i][j][k];
                }
                dp[i + 1][5][k + 1] += dp[i][j][k];
                if((j & 2) || (j & 4)){
                    dp[i + 1][4][k + 1] += dp[i][j][k];
                }
                if((j & 1) || (j & 2)){
                    dp[i + 1][3][k + 1] += dp[i][j][k];
                }
                if((j & 1) || (j & 2)){
