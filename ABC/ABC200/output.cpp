...
int main(){
  int n = 3;
  vector<vector<int>> values(n * n * n, vector<int>(4, 0));

  for(int i = 0; i < n; i++){
    for(int j = 0; j < n; j++){
      for(int k = 0; k < n; k++){
        values[i * n * n + j * n + k][0] = i + j + k + 3;
        values[i * n * n + j * n + k][1] = i + 1;
        values[i * n * n + j * n + k][2] = j + 1;
        values[i * n * n + j * n + k][3] = k + 1;
      }
    }
  }
  sort(values.begin(), values.end());
  for(int i = 0; i < values.size(); i++){
    cout << values[i][0] << " " << 3 - values[i][1] << " " << 3 - values[i][2] << " " << 3 - values[i][3] << endl;
  }
}