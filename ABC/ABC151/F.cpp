...
inline Line Vertical_Bisector(Vec2 A, Vec2 B) {// �����񓙕���
	A = (A + B) / 2;
	return Line(B - A, (B - A).dot(A));
}
inline Vec2 Projection(Vec2 P, Line L) {// �ˉe
	return P + L.norm * (L.dot - L.norm.dot(P));
}
inline Vec2 Intersection(Line L1, Line L2) {// 2�����̌�_
	if (abs(L1.norm.cross(L2.norm)) < EPS) {// ���s�������ꍇ
...
inline Circle Circumscribed_Circle(Vec2 A, Vec2 B, Vec2 C) {// �O�ډ~
	Circle D;
...
inline Circle Circumscribed_Circle(Triangle T) {// �O�ډ~
	return Circumscribed_Circle(T.A, T.B, T.C);
}
inline Circle Inscribed_Circle(Triangle T) {// ���ډ~
...
inline Circle Inscribed_Circle(Vec2 A, Vec2 B, Vec2 C) {// ���ډ~
...
inline vector<Vec2> Intersection(Circle C, Line L) {// �~�ƒ����̌�_
...
inline vector<Vec2> Intersection(Circle C1, Circle C2) {// �~�Ɖ~�̌�_
...
inline pair<Vec2*, int> convex_hull(vector<Vec2>& P) {// �ʕ�
	int N = (int)P.size();
	vector<Vec2> v;
	{
...
Circle SmallestEnclosingDisk(Vec2* P0, int N) {// �ŏ���܉~
	vector<Vec2> P;
	for (int i = 0; i < N; i++) {
		P.push_back(P0[i]);
	}
	...
	while (stack.size() > 0) {
		...
		if (M == 0 || v.size() == 3) {
			...
			case 0:
				...
				break;
			case 1:
				D.center = P[v[0]], D.r2 = 0;
				break;
			case 2:
			{
				D.center = (P[v[0]] + P[v[1]]) / 2;
				D.r2 = (P[v[0]] - D.center).length2();
			}
			break;
			default:
...
Circle SmallestEnclosingDisk(vector<Vec2>& points) {// �ŏ���܉~
...
int main(){
  ...
  if(n == 2){
    ...
    ld x2, y2; cin >> x2 >> y2;
    x -= x2, y -= y2;
    cout << fs(12) << sqrt(x * x + y * y) / 2.0 << endl;
    return 0;
  }
  vector<Vec2> p(n);
  for(int i = 0; i < n; i++){
  ...
  auto c = SmallestEnclosingDisk(hull.first, hull.second);
  cout << fs(12) << c.r << endl;
}